/**
 * MCP tool for retrieving categories tied to a business within an event
 */

import { ArobidClient, ArobidError } from '../client/arobidClient.js';

export interface GetCategoriesByBusinessInput {
  eventId: number;
  businessId: number;
  language?: string;
  currencyId?: number;
  requestId?: string;
  deviceId?: string;
}

export interface GetCategoriesByBusinessResponse {
  [key: string]: unknown;
}

function validateInput(input: unknown): GetCategoriesByBusinessInput {
  if (!input || typeof input !== 'object') {
    throw new Error('Input must be an object');
  }

  const params = input as Record<string, unknown>;

  const requireInt = (key: string): number => {
    const value = params[key];
    if (typeof value !== 'number' || !Number.isInteger(value)) {
      throw new Error(`${key} is required and must be an integer`);
    }
    return value;
  };

  const result: GetCategoriesByBusinessInput = {
    eventId: requireInt('eventId'),
    businessId: requireInt('businessId'),
  };

  if (params.language !== undefined) {
    if (typeof params.language !== 'string') {
      throw new Error('language must be a string if provided');
    }
    result.language = params.language.trim();
  }

  if (params.currencyId !== undefined) {
    if (
      typeof params.currencyId !== 'number' ||
      params.currencyId < 1 ||
      !Number.isInteger(params.currencyId)
    ) {
      throw new Error('currencyId must be a positive integer if provided');
    }
    result.currencyId = params.currencyId;
  }

  if (params.requestId !== undefined) {
    if (typeof params.requestId !== 'string') {
      throw new Error('requestId must be a string if provided');
    }
    result.requestId = params.requestId.trim();
  }

  if (params.deviceId !== undefined) {
    if (typeof params.deviceId !== 'string') {
      throw new Error('deviceId must be a string if provided');
    }
    result.deviceId = params.deviceId.trim();
  }

  return result;
}

function buildHeaders(input: GetCategoriesByBusinessInput): Record<string, string> {
  const headers: Record<string, string> = {
    accept: 'application/json',
    'accept-language': input.language || 'en-US,en;q=0.9,vi;q=0.8',
    language: input.language || 'en',
    currencyid: input.currencyId?.toString() || '1',
  };

  if (input.requestId) headers['requestid'] = input.requestId;
  if (input.deviceId) headers['deviceid'] = input.deviceId;

  return headers;
}

function buildQuery(input: GetCategoriesByBusinessInput): string {
  const params = new URLSearchParams();
  params.set('EventId', input.eventId.toString());
  params.set('BusinessId', input.businessId.toString());
  return params.toString();
}

export async function getCategoriesByBusiness(
  client: ArobidClient,
  input: unknown
): Promise<GetCategoriesByBusinessResponse> {
  const validatedInput = validateInput(input);

  try {
    const query = buildQuery(validatedInput);
    const endpoint = `/tradexpo/api/category/get-all-categories-by-business-id?${query}`;
    const headers = buildHeaders(validatedInput);

    console.error(
      `[getCategoriesByBusiness] Fetching categories for event ${validatedInput.eventId}, business ${validatedInput.businessId}`
    );

    const response = await client.get<GetCategoriesByBusinessResponse>(endpoint, headers);
    console.error('[getCategoriesByBusiness] Successfully fetched categories');
    return response;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      const arobidError = error as ArobidError;
      console.error(
        `[getCategoriesByBusiness] API Error:\n` +
          `  Status Code: ${arobidError.statusCode}\n` +
          `  Error Code: ${arobidError.code || 'N/A'}\n` +
          `  Message: ${arobidError.message}`
      );
      throw new Error(
        `Failed to get categories by business: ${arobidError.message}${
          arobidError.code ? ` (${arobidError.code})` : ''
        }`
      );
    }

    console.error(
      `[getCategoriesByBusiness] Unexpected Error:\n` +
        `  Type: ${error instanceof Error ? error.constructor.name : typeof error}\n` +
        `  Message: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw new Error(
      `Failed to get categories by business: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}


