/**
 * MCP tool for unsubscribing from a workspace package
 */

import { ArobidClient, ArobidError } from '../client/arobidClient.js';

export interface UnsubscribeWorkspacePackageInput {
  id: number;
  language?: string;
  currencyId?: number;
  requestId?: string;
...
export interface UnsubscribeWorkspacePackageResponse {
  [key: string]: unknown;
}

function validateInput(input: unknown): UnsubscribeWorkspacePackageInput {
  ...
  const result: UnsubscribeWorkspacePackageInput = {
    id: params.id,
  };
...
function buildHeaders(input: UnsubscribeWorkspacePackageInput): Record<string, string> {
...
export async function unsubscribeWorkspacePackage(
  client: ArobidClient,
  input: unknown
): Promise<UnsubscribeWorkspacePackageResponse> {
  const validatedInput = validateInput(input);

  try {
    const endpoint = `/tradexpo/api/workspace/unsubscribe/${validatedInput.id}`;
    const headers = buildHeaders(validatedInput);

    console.error(`[unsubscribeWorkspacePackage] Unsubscribing package ${validatedInput.id}`);
    const response = await client.delete<UnsubscribeWorkspacePackageResponse>(endpoint, headers);
    console.error('[unsubscribeWorkspacePackage] Successfully unsubscribed package');
    return response;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      const arobidError = error as ArobidError;
      console.error(
        `[unsubscribeWorkspacePackage] API Error:\n` +
          `  Status Code: ${arobidError.statusCode}\n` +
          `  Error Code: ${arobidError.code || 'N/A'}\n` +
          `  Message: ${arobidError.message}`
      );
      throw new Error(
        `Failed to unsubscribe workspace package: ${arobidError.message}${
    ...
    console.error(
      `[unsubscribeWorkspacePackage] Unexpected Error:\n` +
        `  Type: ${error instanceof Error ? error.constructor.name : typeof error}\n` +
        `  Message: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw new Error(
      `Failed to unsubscribe workspace package: ${
