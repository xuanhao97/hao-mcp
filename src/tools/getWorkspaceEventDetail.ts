/**
 * MCP tool for retrieving event details within the workspace context
 */

import { ArobidClient, ArobidError } from '../client/arobidClient.js';

export interface GetWorkspaceEventDetailInput {
  eventId: number;
  language?: string;
  currencyId?: number;
  requestId?: string;
...
export interface GetWorkspaceEventDetailResponse {
  [key: string]: unknown;
}

function validateInput(input: unknown): GetWorkspaceEventDetailInput {
  ...
  const result: GetWorkspaceEventDetailInput = {
...
function buildHeaders(input: GetWorkspaceEventDetailInput): Record<string, string> {
...
export async function getWorkspaceEventDetail(
  client: ArobidClient,
  input: unknown
): Promise<GetWorkspaceEventDetailResponse> {
  const validatedInput = validateInput(input);

  try {
    const endpoint = `/tradexpo/api/workspace/get-detail-event/${validatedInput.eventId}`;
    const headers = buildHeaders(validatedInput);

    console.error(
      `[getWorkspaceEventDetail] Fetching workspace event detail for event ${validatedInput.eventId}`
    );

    const response = await client.get<GetWorkspaceEventDetailResponse>(endpoint, headers);
    console.error('[getWorkspaceEventDetail] Successfully fetched workspace event detail');
    return response;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      const arobidError = error as ArobidError;
      console.error(
        `[getWorkspaceEventDetail] API Error:\n` +
          `  Status Code: ${arobidError.statusCode}\n` +
          `  Error Code: ${arobidError.code || 'N/A'}\n` +
          `  Message: ${arobidError.message}`
      );
      throw new Error(
        `Failed to get workspace event detail: ${arobidError.message}${
    ...
    console.error(
      `[getWorkspaceEventDetail] Unexpected Error:\n` +
        `  Type: ${error instanceof Error ? error.constructor.name : typeof error}\n` +
        `  Message: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw new Error(
      `Failed to get workspace event detail: ${
