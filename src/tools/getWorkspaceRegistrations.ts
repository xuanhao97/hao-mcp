/**
 * MCP tool for retrieving registrations within the workspace dashboard
 */

import { ArobidClient, ArobidError } from '../client/arobidClient.js';

export interface GetWorkspaceRegistrationsInput {
  businessId?: number;
  eventId?: number;
  search?: string;
  isRegisted?: boolean;
...
export interface GetWorkspaceRegistrationsResponse {
  [key: string]: unknown;
}

function validateInput(input: unknown): GetWorkspaceRegistrationsInput {
  if (!input || typeof input !== 'object') {
    return {};
  }

  const params = input as Record<string, unknown>;
  const result: GetWorkspaceRegistrationsInput = {};

  const copyOptionalInt = (key: keyof GetWorkspaceRegistrationsInput) => {
...
function buildHeaders(input: GetWorkspaceRegistrationsInput): Record<string, string> {
...
function buildQuery(input: GetWorkspaceRegistrationsInput): string {
...
export async function getWorkspaceRegistrations(
  client: ArobidClient,
  input: unknown
): Promise<GetWorkspaceRegistrationsResponse> {
  const validatedInput = validateInput(input);

  try {
    const query = buildQuery(validatedInput);
    const endpoint = `/tradexpo/api/workspace/get-all-registration${query ? `?${query}` : ''}`;
    const headers = buildHeaders(validatedInput);

    console.error('[getWorkspaceRegistrations] Fetching workspace registrations');
    const response = await client.get<GetWorkspaceRegistrationsResponse>(endpoint, headers);
    console.error('[getWorkspaceRegistrations] Successfully fetched workspace registrations');
    return response;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      const arobidError = error as ArobidError;
      console.error(
        `[getWorkspaceRegistrations] API Error:\n` +
          `  Status Code: ${arobidError.statusCode}\n` +
          `  Error Code: ${arobidError.code || 'N/A'}\n` +
          `  Message: ${arobidError.message}`
      );
      throw new Error(
        `Failed to get workspace registrations: ${arobidError.message}${
    ...
    console.error(
      `[getWorkspaceRegistrations] Unexpected Error:\n` +
        `  Type: ${error instanceof Error ? error.constructor.name : typeof error}\n` +
        `  Message: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw new Error(
      `Failed to get workspace registrations: ${
