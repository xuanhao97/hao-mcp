/**
 * MCP tool for retrieving main packages for an event inside the workspace
 */

import { ArobidClient, ArobidError } from '../client/arobidClient.js';

export interface GetWorkspaceMainPackagesInput {
  eventId: number;
  language?: string;
  currencyId?: number;
  requestId?: string;
...
export interface GetWorkspaceMainPackagesResponse {
  [key: string]: unknown;
}

function validateInput(input: unknown): GetWorkspaceMainPackagesInput {
  ...
  const result: GetWorkspaceMainPackagesInput = {
...
function buildHeaders(input: GetWorkspaceMainPackagesInput): Record<string, string> {
...
export async function getWorkspaceMainPackages(
  client: ArobidClient,
  input: unknown
): Promise<GetWorkspaceMainPackagesResponse> {
  const validatedInput = validateInput(input);

  try {
    const endpoint = `/tradexpo/api/workspace/get-all-main/${validatedInput.eventId}`;
    const headers = buildHeaders(validatedInput);

    console.error(
      `[getWorkspaceMainPackages] Fetching main packages for event ${validatedInput.eventId}`
    );

    const response = await client.get<GetWorkspaceMainPackagesResponse>(endpoint, headers);
    console.error('[getWorkspaceMainPackages] Successfully fetched main packages');
    return response;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      const arobidError = error as ArobidError;
      console.error(
        `[getWorkspaceMainPackages] API Error:\n` +
          `  Status Code: ${arobidError.statusCode}\n` +
          `  Error Code: ${arobidError.code || 'N/A'}\n` +
          `  Message: ${arobidError.message}`
      );
      throw new Error(
        `Failed to get workspace main packages: ${arobidError.message}${
    ...
    console.error(
      `[getWorkspaceMainPackages] Unexpected Error:\n` +
        `  Type: ${error instanceof Error ? error.constructor.name : typeof error}\n` +
        `  Message: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw new Error(
      `Failed to get workspace main packages: ${
