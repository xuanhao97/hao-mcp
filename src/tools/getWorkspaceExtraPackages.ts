/**
 * MCP tool for retrieving extra packages for an event inside the workspace
 */

import { ArobidClient, ArobidError } from '../client/arobidClient.js';

export interface GetWorkspaceExtraPackagesInput {
  eventId: number;
  language?: string;
  currencyId?: number;
  requestId?: string;
...
export interface GetWorkspaceExtraPackagesResponse {
  [key: string]: unknown;
}

function validateInput(input: unknown): GetWorkspaceExtraPackagesInput {
  ...
  const result: GetWorkspaceExtraPackagesInput = {
...
function buildHeaders(input: GetWorkspaceExtraPackagesInput): Record<string, string> {
...
export async function getWorkspaceExtraPackages(
  client: ArobidClient,
  input: unknown
): Promise<GetWorkspaceExtraPackagesResponse> {
  const validatedInput = validateInput(input);

  try {
    const endpoint = `/tradexpo/api/workspace/get-all-extra/${validatedInput.eventId}`;
    const headers = buildHeaders(validatedInput);

    console.error(
      `[getWorkspaceExtraPackages] Fetching extra packages for event ${validatedInput.eventId}`
    );

    const response = await client.get<GetWorkspaceExtraPackagesResponse>(endpoint, headers);
    console.error('[getWorkspaceExtraPackages] Successfully fetched extra packages');
    return response;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      const arobidError = error as ArobidError;
      console.error(
        `[getWorkspaceExtraPackages] API Error:\n` +
          `  Status Code: ${arobidError.statusCode}\n` +
          `  Error Code: ${arobidError.code || 'N/A'}\n` +
          `  Message: ${arobidError.message}`
      );
      throw new Error(
        `Failed to get workspace extra packages: ${arobidError.message}${
    ...
    console.error(
      `[getWorkspaceExtraPackages] Unexpected Error:\n` +
        `  Type: ${error instanceof Error ? error.constructor.name : typeof error}\n` +
        `  Message: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw new Error(
      `Failed to get workspace extra packages: ${
