/**
 * MCP tool for retrieving a business profile inside the organizer workspace
 */

import { ArobidClient, ArobidError } from '../client/arobidClient.js';

export interface GetWorkspaceBusinessDetailInput {
  eventId: number;
  businessId?: number;
  language?: string;
  currencyId?: number;
...
export interface GetWorkspaceBusinessDetailResponse {
  [key: string]: unknown;
}

function validateInput(input: unknown): GetWorkspaceBusinessDetailInput {
  ...
  const result: GetWorkspaceBusinessDetailInput = {
...
function buildHeaders(input: GetWorkspaceBusinessDetailInput): Record<string, string> {
...
function buildQuery(input: GetWorkspaceBusinessDetailInput): string {
...
export async function getWorkspaceBusinessDetail(
  client: ArobidClient,
  input: unknown
): Promise<GetWorkspaceBusinessDetailResponse> {
  const validatedInput = validateInput(input);

  try {
    const query = buildQuery(validatedInput);
    const endpoint = `/tradexpo/api/workspace/get-detail-business/${validatedInput.eventId}${
      query ? `?${query}` : ''
    }`;
    const headers = buildHeaders(validatedInput);

    console.error(
      `[getWorkspaceBusinessDetail] Fetching business detail for event ${validatedInput.eventId}`
    );

    const response = await client.get<GetWorkspaceBusinessDetailResponse>(endpoint, headers);
    console.error('[getWorkspaceBusinessDetail] Successfully fetched business detail');
    return response;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      const arobidError = error as ArobidError;
      console.error(
        `[getWorkspaceBusinessDetail] API Error:\n` +
          `  Status Code: ${arobidError.statusCode}\n` +
          `  Error Code: ${arobidError.code || 'N/A'}\n` +
          `  Message: ${arobidError.message}`
      );
      throw new Error(
        `Failed to get workspace business detail: ${arobidError.message}${
    ...
    console.error(
      `[getWorkspaceBusinessDetail] Unexpected Error:\n` +
        `  Type: ${error instanceof Error ? error.constructor.name : typeof error}\n` +
        `  Message: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    throw new Error(
      `Failed to get workspace business detail: ${
